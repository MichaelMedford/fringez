...
def gather_fringe_maps_serial(N_samples):
    ...
    Masking will be done in one of two ways. For each science image, if a
    mask image is present ending in '_mskimg.fits' instead of '_sciimg.fits'
    (following the ZTF convention) then the mask image will be read in. A
    zero-mask will be applied in the science image to all pixels in the
    mask image with a non-zero value. If a mask image is not present, all
    pixels that are +-5 sigma in the science image will be set to zero.

    A file named 'fringe_maglimits.txt' must be present with format:
        fringe_filename,maglimit
        <fringe_filename>,<maglimit>
        <fringe_filename>,<maglimit>
        <fringe_filename>,<maglimit>
        ...
    ...
    print('%i image on disk | %i samples -> ~%i images per sample' % (N_images, N_samples, N_images_per_sample))

    # Load the fringe maps samples
    fringe_maps = []
    for id_sample in range(N_samples):
        ...
        for j, idx in enumerate(idx_sample):
